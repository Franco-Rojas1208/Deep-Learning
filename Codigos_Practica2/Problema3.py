...
# Parte opcional (SVM y LogisticRegression de scikit-learn)
RUN_SKLEARN = True
N_SKLEARN = 10000              # subconjunto de entrenamiento (por tiempo de computo)
...
def backward(cache, Y, params, lam):
    X, H, Yhat = cache["X"], cache["H"], cache["Yhat"]
    N = X.shape[0]

    # --- nodo costo: dL/dYhat -----------------------------------------------
    dYhat = (2.0 / N) * (Yhat - Y)                     # (N,10)

    # --- activacion lineal de la salida: dYhat/dZ2 = 1 ----------------------
    dZ2 = dYhat                                        # (N,10)
...
def evaluate(X, Y, y, params, lam, batch=2000):
    N = X.shape[0]
    total_sq = 0.0
    correct = 0
    for i in range(0, N, batch):
        Yhat, _ = forward(X[i:i + batch], params)
        total_sq += np.sum((Yhat - Y[i:i + batch]) ** 2)
...
def train(X_tr, Y_tr, y_tr, X_te, Y_te, y_te,
          ...
          lam=LAMBDA_L2, batch_size=BATCH_SIZE, epochs=EPOCHS, seed=SEED, verbose=True):
    ...
    for ep in range(epochs):
        ...
        ltr, atr = evaluate(X_tr[idx_eval], Y_tr[idx_eval], y_tr[idx_eval], params, lam)
        lte, ate = evaluate(X_te, Y_te, y_te, params, lam)
...
def plot_curves(hist, path):
    ...
    fig.suptitle("Ejercicio 3 - red 3072-100(sigmoide)-10(lineal), MSE + L2")
    fig.tight_layout()
    fig.savefig(path, dpi=140)
    plt.close(fig)
    print(f"  figura guardada en {path}")
...
# =============================================================================
# 7. PARTE OPCIONAL: clasificadores lineales de scikit-learn
# =============================================================================

def sklearn_baselines(X_tr, y_tr, X_te, y_te, n_train=N_SKLEARN, seed=SEED):
    from sklearn.svm import LinearSVC
    from sklearn.linear_model import LogisticRegression

    rng = np.random.default_rng(seed)
    idx = rng.choice(X_tr.shape[0], size=min(n_train, X_tr.shape[0]), replace=False)
    Xs, ys = X_tr[idx], y_tr[idx]

    out = {}

    t0 = time.time()
    svm = LinearSVC(C=1e-3, max_iter=3000)
    svm.fit(Xs, ys)
    out["SVM lineal"] = svm.score(X_te, y_te)
    print(f"  LinearSVC          acc_test={out['SVM lineal']:.4f}  [{time.time()-t0:.0f}s]")

    t0 = time.time()
    logreg = LogisticRegression(C=1e-2, max_iter=200)
    logreg.fit(Xs, ys)
    out["Regresion logistica"] = logreg.score(X_te, y_te)
    print(f"  LogisticRegression acc_test={out['Regresion logistica']:.4f}  [{time.time()-t0:.0f}s]")
...
def main():
    ...
    print("=" * 68)
    print("Practica 2 - Ejercicio 3: red de dos capas para CIFAR-10")
    print("=" * 68)

    # --- 1) datos -----------------------------------------------------------
    print("Cargando CIFAR-10...")
    x_tr, y_tr, x_te, y_te = load_cifar10()
    plot_sample_images(x_tr, y_tr, n=3, path=os.path.join(OUT_DIR, "ej3_muestras.png"))
    ...
    # --- 3) graficos --------------------------------------------------------
    plot_curves(hist, os.path.join(OUT_DIR, "ej3_curvas.png"))

    # --- 4) parte opcional --------------------------------------------------
    if RUN_SKLEARN:
        print("\nClasificadores lineales de scikit-learn "
              f"(subconjunto de {N_SKLEARN} ejemplos):")
        results = sklearn_baselines(X_tr, y_tr, X_te, y_te)
        results = {"Red 3072-100-10": acc_red, **results}
        plot_comparison(results, os.path.join(OUT_DIR, "ej3_comparacion.png"))
